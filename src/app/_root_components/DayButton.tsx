...
export function DayButton({ right, title, day, discount, children, href, ariaLabel }: DayButtonProps) {
  return (
    <div aria-label={ariaLabel} className={`w-full flex ${right ? 'justify-end' : 'justify-start'}`}>
      <Link href={href} className="day-link flex justify-between bg-white w-[90%] md:w-[70%] rounded-md overflow-hidden hover:bg-secondary">
        <div aria-label={"Informacion de link de dia"} className="px-3 py-6">
